...
function UserPage({ users, roles, entities, onCreate, onUpdate, onDelete }) {
  ...
  const handleResetPassword = async (userId, userName) => {
    const result = await Swal.fire({
      ...
      text: `Password untuk "${userName}" akan diubah menjadi kode acak baru.`,
      icon: 'question',
      showCancelButton: true,
      confirmButtonText: 'Ya, Generate Baru!',
      cancelButtonText: 'Batal',
      confirmButtonColor: '#f59e0b', // Warna oranye
    });
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (res.newPassword) {
          // Tampilkan Popup Password Baru
          await Swal.fire({
            title: 'Password Berhasil Direset!',
            html: `
              <p class="text-slate-600 mb-2">Silakan salin password baru ini:</p>
              <div class="bg-slate-100 p-4 rounded-lg border border-slate-300 shadow-inner mb-2">
                <span class="font-mono text-2xl font-bold tracking-widest text-slate-800 select-all" style="display:inline-block;">${res.newPassword.trim()}</span>
              </div>
              <p class="text-xs text-slate-400">*(User bisa mengubahnya nanti saat login)*</p>
            `,
            icon: 'success',
    ...
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-1 space-y-6">
            {(hasPermission('manage_users') || hasPermission('create_users') || hasPermission('update_users')) ? (
