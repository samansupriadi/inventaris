...
function AssetTable({
  ...
  const handleDeleteClick = (asset) => setDeleteModal({ open: true, asset });
  ...
  return (
    <>
      <div className="bg-white shadow-sm border border-slate-100 rounded-xl overflow-hidden relative font-sans">
        ...
        {/* TOAST */}
        {toast.show && (
          <div className={`fixed top-5 right-5 z-[70] px-4 py-3 rounded-lg shadow-lg border flex items-center gap-3 animate-fade-in ${
            ...
            <table className="w-full text-xs md:text-sm text-left border-collapse">
              <thead className="bg-[#009846]/5 border-b border-[#009846]/10 text-[#007033] uppercase text-[11px] tracking-wider font-semibold">
                <tr>
                  ...
                  <th className="p-4 whitespace-nowrap">Nilai Buku & Beli</th>
                    ...
                    <tr key={a.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <td className="p-4 align-top text-slate-600 font-medium">
                        ...
                        {a.useful_life > 0 ? (
                          ...
                          <div className="text-[10px] text-slate-300 mt-0.5 italic">
                            Non-Depreciable
                          </div>
                        )}
                      </td>
                      ...
                      <td className="p-4 align-top text-center">
                        <div className="relative group/photo w-10 h-10 mx-auto">
                          {a.photo_url ? (
                            <button onClick={() => onPreviewPhoto?.(`${API_BASE_URL}${a.photo_url}`)} className="w-10 h-10 rounded-lg border border-slate-200 overflow-hidden hover:opacity-90 transition-opacity bg-slate-50 shadow-sm">
                              <img src={`${API_BASE_URL}${a.photo_url}`} alt={a.name} className="w-full h-full object-cover" />
                            </button>
                          ) : (
                            <div className="w-10 h-10 flex items-center justify-center border border-slate-200 rounded-lg text-[8px] text-gray-400 bg-slate-50">No IMG</div>
                          )}
                          <label className="absolute -bottom-2 -right-2 cursor-pointer bg-white rounded-full p-1 shadow-md border border-slate-100 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-orange-50" title="Upload Foto">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#F68D2E" className="w-3 h-3"><path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" /></svg>
                            <input type="file" accept="image/*" onChange={(e) => onUploadPhoto?.(a.id, e)} className="hidden" />
                          </label>
                        </div>
                      </td>
                          ...
                          <div className="relative" onClick={(e) => e.stopPropagation()}>
                            ...
                            {openMenuId === a.id && (
                              <div className="absolute right-0 mt-2 w-40 bg-white rounded-xl shadow-xl border border-slate-100 z-50 overflow-hidden animate-fade-in-up origin-top-right ring-1 ring-black/5">
                                <div className="py-1">
                                  <button onClick={() => { setOpenMenuId(null); handlePrintSingle(a); }} className="w-full text-left px-4 py-2.5 text-xs text-slate-600 hover:bg-slate-50 hover:text-[#009846] flex items-center gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in" onClick={() => setDeleteModal({ open: false, asset: null })}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all scale-100 border border-slate-100" onClick={(e) => e.stopPropagation()}>
            <div className="bg-red-50 p-6 flex flex-col items-center text-center gap-3 border-b border-red-50">
              ...
              <div><h3 className="text-lg font-bold text-slate-800">Hapus Aset Ini?</h3><p className="text-xs text-slate-500 mt-1">Data akan dipindahkan ke sampah.</p></div>
            </div>
            <div className="p-5 text-sm text-slate-600 bg-white">
              <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 flex items-center gap-3">
                <div className="w-10 h-10 bg-white rounded border border-slate-100 flex items-center justify-center text-xl">📦</div>
                <div className="overflow-hidden"><div className="font-bold text-slate-800 truncate">{deleteModal.asset?.name}</div><div className="font-mono text-xs text-slate-500 truncate">{deleteModal.asset?.code}</div></div>
              </div>
            </div>
            <div className="flex gap-3 p-5 pt-0">
              <button onClick={() => setDeleteModal({ open: false, asset: null })} className="flex-1 px-4 py-2.5 text-sm font-medium text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50">Batal</button>
              <button onClick={handleConfirmDelete} className="flex-1 px-4 py-2.5 text-sm font-medium text-white bg-red-600 border border-red-600 rounded-xl hover:bg-red-700 shadow-md">Ya, Hapus</button>
            </div>
          </div>
        </div>
      )}
